...
** 更好的选择 proxychains
   ...
   - [[https://sites.google.com/a/pickdreams.org/snail-library/Home/yong-tsocks-heproxychains-dai-lilinux-xia-suo-you-ruan-jian][用tsocks和proxychains代理Linux下所有软件 - 蜗牛图书馆]]
...
* DONE system(3) 与 SIGCHLD 信号                                    :linux:c:
  ...
  - 设置当前线程的 SIGCHLD 信号为阻塞（SIG_BLOCK）状态
...
** 在整个应用中忽略 SIGCHLD 信号会有负作用
   ...
   [[http://stackoverflow.com/a/25039605/802708][c - system() function while SIGCHLD is ignored - Stack Overflow]] 或 man wait(2) 说明了原因：
   #+begin_quote
...
** 级连同步
   ...
   当 Fail-Over 发生后级连同步会失效。

** 内网流量外网化

   DataCenter 1 中的 Master 与 Slave 可配置为通过内网同步数据（bind 指定内网 IP），但是 DataCenter 2 上部署的应用要通过 Sentinel 访问 DataCenter 1 上的 Master 或 Slave，所以 DataCenter 1 中的 Master 与 Slave 也不得不配置为通过外网同步数据。

   客户端预先获得所有 redis 实例的内外网 IP 信息，还是可以走内网进行优化的，Master 与 Slave 间的同步流量就无能为力了。
   ...
   它们解决了以下两个问题（见 [[http://redis.io/topics/sentinel#sentinel-docker-nat-and-possible-issues][Redis Sentinel Documentation – Redis - Sentinel, Docker, NAT, and possible issues]]）
   #+begin_quote
   ...
   或许可以在系统网络层面来解决这个问题，利用 iptables 的 port forwarding，将外网流量内网化，当我们通过外网 IP 访问同机房的 Redis 服务器时，将它转到 Redis 服务器的内网 IP。
