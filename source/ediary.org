...
* DONE 解决个人用户目录做为 Web 服务根目录的权限问题        :web:linux:nginx:
  ...
  nginx 配置 Web 服务器的根目录为个人用户目录，这样修复代码后刷新浏览器就可以看到效果。
  ...
  使用 linux 命令确认问题是由 linux 的用户权限系统引起
  #+begin_example
...
- 将用户目录挂载到 Web 帐号目录下
  ...
  Web 服务器和日常开发可以兼顾，是个两全其美方案。

  
  在 /etc/fstab 添加挂载项以支持开机自动挂载

  #+begin_example
    /home/tangxinfa/projects/www.example.com    /var/www/www.example.com     none    bind,ro    0 0
  #+end_example
