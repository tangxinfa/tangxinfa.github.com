...
** 合并特性分支的提交记录，便于上游开发人员做 Code Review
...
** 从特性分支创建 Pull requests 提交到上游分支
   ...
   如果已经创建 =Pull request= 别人 Review 时说少了 =Signed-off-by= ，则可以先回滚到第一个提交：
   #+begin_src sh
     git reset --soft 提交ID
   #+end_src
...
* TODO 使用 glide 进行 GO 项目包管理                                     :go:
  ...
  =glide= 将包安装在 =vendor= 目录下，由于通过非全路径引入的包不会在 =vendor= 目录下进行查找，所以也就引申出下面几条 glide 最佳实践。

** glide 最佳实践

*** 在标准位置进行项目开发

    也就是将开发中的项目代码放到 =$GOPATH/src/<project>= ，可以直接使用 =go get= 拉取项目代码：

**** 使用 https（帐号密码认证）

     =go get= 进行 =git clone= 时允许输入帐号密码（默认不充许）
     #+begin_src sh
...
**** 使用 git（SSH key 认证）
     ...
     =go get= 进行 =git clone= 时优先进行 =SSH key= 认证 （默认 =https= ），这样通过上传 =SSH key= 可以免密码
     #+begin_src sh
...
**** 拉取代码并切到 dev 分支
     #+begin_src sh
       go get gitlab.example.com/myapp
       cd $GOPATH/src/gitlab.example.com/myapp
       git checkout dev
     #+end_src
