...
* DONE package.json文件dependencies中的各种版本号形式                  :node:
  ...
  一般自已写 =package.json= 时，图省事版本号会用 =*= ，想想也是很危险的，指不定哪天依赖的包不在向后兼容，程序运行估计就有问题了。
...
** 版本号解析示例
   ...
   - ^1.0.0 :: 匹配 >=1.0.0 且 <2.0.0的版本。
               ...
               ^1.1.0 匹配 >=1.1.0 且 <1.2.0

               ^0.0.3 匹配 >=0.0.3 且 <0.0.4

               ^1.1.1 匹配 >=1.1.1 且 <2.0.0
   ...
   - ^5.x :: 匹配 >=5.0.0 且 <6.0.0。
    ...
    - >=3.0.0 :: 同字面意义 >=3.0.0。
                 ...
                 其它操作符有 =<= =<\== =>= =>\== =\== ，多个表达式之间用空格分隔并示并集，用 =||= 分隔交集。
...
* DONE node.js写日志堵塞问题                                           :node:
  ...
  #+end_example
   ...
   所以我最终要找的，不仅仅是一个异步写日志文件的模块，还要先减少磁盘写入量，或者日志不直接写到磁盘文件，而是发送到一个日志服务。
