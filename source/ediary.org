...
* DONE system(3) 与 SIGCHLD 信号                                    :linux:c:
  ...
  - fork(3)
    ...
    + 子进程
      ...
      恢复 SIGCHLD 信号处理，然后 execl(3)。

    + 父进程

      等待子进程（waitpid(3)）结束，恢复当前线程的 SIGCHLD 信号处理。
...
** 在整个应用中阻塞 SIGCHLD 信号可能导致孙进程僵尸
   ...
   system(3) 在替换进程为新程序（execl(3)）之前，会恢复为默认的信号掩码（SIGCHLD 处于阻塞状态），可能干扰子进程运行，
   如通过 system(3) 启动了 sshd，有用户尝试登录，sshd 会再 fork(3) 一个孙进程，然后在 SIGCHLD 信号处理函数中通过 waitpid(3) 回收孙进程，但是从父进程（调用 system(3)的进程）继承而来信号掩码阻塞了 SIGCHLD 信号，导致孙进程结束后成为僵尸进程。

   相关 BUG 报告
   [[https://bugzilla.mindrot.org/show_bug.cgi?id=271][271 – SSHD should unblock SIGCHLD - POSIX signal blocks survive exec()]]

   在调用 system(3) 前取消对 SIGCHLD 的阻塞，调用返回后恢复对 SIGCHLD 的阻塞，可以解决这个问题
   #+begin_src C
   ...
   考虑到还有 popen(3)/pclose(3) 也可以用来创建子进程，所以还有很多替换工作。

** 折衷方案

   放任 SIGCHLD 不管会任由它将其它线程的系统调用中断，而堵塞它也可能因为僵尸孙进程而影响系统的稳定性，所以只能选择忽略它，当需要通过 system(3) 的返回值确定命令执行是否成功时，可以改用 popen(3)/pclose(3) 通过读取命令的标准输出进行判断。
