...
- 使用 =gdb= 获取崩溃调用栈
  ...
  会获得类似以下输出
  #+begin_example
    $ gdb -q --nh --nx --batch -ex bt --core=./app.core  
    [New LWP 17045]
    [New LWP 17053]
    [New LWP 17052]
    [New LWP 17051]
    ...
    #0  0x000056355d1fd9a4 in ?? ()
    [Current thread is 1 (LWP 17045)]
    #0  0x000056355d1fd9a4 in ?? ()
...
- 使用 =eu-addr2line= 获取地址对应的代码行
  ...
  #+begin_example
    $ eu-addr2line -e ./app_debug --core ./app.core 0x000056355d1fd9a4
    /home/tangxinfa/Examples/c/app.c:17
  #+end_example
...
- 问题：依赖于 core 文件
  ...
  一般来说崩溃可能发生在不受控的设备里，如果崩溃产生的 core 文件过大则上传会耗时过长，
  甚至有的网络环境是按流量收费的，需要找到一种方法直接从可执行程序或其它信息获取到崩溃的源代码行。
...
* TODO linux 下调试剥去调试信息的程序崩溃：使用 Map 文件              :linux:

  本文为前一篇文章《[[http://blog.kankanan.com/article/linux-4e0b8c038bd5526553bb8c038bd54fe1606f76847a0b5e8f5d296e83.html][linux 下调试剥去调试信息的程序崩溃]]》的续篇。

  之前调研过使用 Map 文件，试验失败，看了一下 Map 文件的内容，其中记载了函数的地
  址，但没有详细的代码行地址，理论上是实现不了从崩溃地址获取源代码行地址的目标的，
  所以采用了比较重的方案（依赖 core 文件及未 strip 的可执行文件），后来一直在查
  资料，通过阅读《[[http://www.cchsu.com/arthur/prg_bg5/map_debug.htm][使用 MAP file 除錯的技巧]]》一文，发现是之前姿势不对。
...
** 参考
   ...
   - [[http://www.cchsu.com/arthur/prg_bg5/map_debug.htm][使用 MAP file 除錯的技巧]]
