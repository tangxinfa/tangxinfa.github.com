...
* DONE process.exit导致的控制台输出不全                                :node:
  ...
  - 解决方案
    ...
    警告：在 =node.js= 开发的一个后台服务（使用 [[https://github.com/Unitech/pm2][pm2]] 管理进程）中， =process.stderr.write= ([[stderr-write]]) 的回调函数未调用，
...
* DONE node.js下进行mqtt实践                               :network:IoT:node:
  ...
  简单起见， =node.js= 服务器端使用 [[https://github.com/mcollina/mosca][mosca]]， 客户端使用 [[https://github.com/mqttjs/MQTT.js][MQTT.js]]。
...
** 客户端与客户端离线通信
   ...
   - 订阅方连接服务器时指定 =clean= 为 =false=
   
   - 发布方发布消息时指定 =qos= 大于 =0= 以及 =retain= 为 =true=
   ...
   #+begin_src js
     ...
     var db = new mosca.persistence.Redis({host: '127.0.0.1', port: 6379, db: 13});
     db.wire(server);
...
** 定期清理日志
   ...
   不建议使用 [[https://www.npmjs.com/package/pm2-logrotate][pm2-logrotate]] ，太多问题了（详见： [[http://blog.kankanan.com/article/63a75236-pm2-768465e55fd765874ef659275c0f.html#sec-1][控制pm2的日志文件大小]] ），还是使用 logrotate 服务靠谱（参见 [[http://pm2.keymetrics.io/docs/usage/log-management/#setting-up-a-native-logrotate][Setting up a native logrotate]]）。
...
* TODO 快乐去哪里了                                                      :反省:
  ...
  从什么时候开始，再也难以开怀大笑。
  需要被逗乐才会笑，所以没事就不自觉得找乐子，可能是笑点越来越低了，找来找去徒烦闷。
  需要被奖励才会满足，所以总想着努力会有回报，却难以从忙忙碌碌中找到存在感。
