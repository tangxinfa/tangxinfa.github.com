...
** 总结
   ...
   使用 =gdb= 从 core 文件获取崩溃调用栈的方案依赖 core 文件，core 的取得会比较
   麻烦。但如果要实现接管操作系统级的 core dump 处理，然后统一上报则会很变得必不
   可少，而且这个方案会非常通用。生产环境需要开启 core dump 以及安装 =gdb= 。
   ...
   行时不依赖未 strip 调试信息的可执行程序。需要进行的手工查找、计算，暂时没有找
   到简化操作的工具。
