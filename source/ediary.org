...
** 优先选择本地 Slave
   ...
   #+begin_src js
     ...
     function resolveSlavePreferLocal (client, callback) {
         client.sentinel('slaves', this.options.name, function (err, result) {
             ...
             callback(null, selectedSlave ? { host: selectedSlave.ip, port: selectedSlave.port, local_node: Boolean(localSlaves.length) } : null);
         });
     };
     ...
     function preferLocalSlave(client) {
         if (client.connector.resolveSlave) {
   ...
   #+begin_src js
     var options = {name: "data", sentinels: sentinels, db: 0, role: "slave"}
     var client = new Redis(options);
     if (preferLocalSlave(client)) {
         logger.warn("prefer local slave on redis sentinel(" + JSON.stringify(options) + ")");
     }
   #+end_src
...
** 优先选择本地结点
   ...
   #+begin_src js
     ...
     if (preferLocal(client)) {
         logger.warn("prefer local on redis sentinel(" + JSON.stringify(options) + ")");
     }
   #+end_src
