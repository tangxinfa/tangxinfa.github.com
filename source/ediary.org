...
** 收集性能指标
   ...
   编译安装
   #+begin_src sh
     gcc -O2 collectd-pm2.c -o /usr/local/bin/collectd-pm2
     chown root:root /usr/local/bin/collectd-pm2
     chmod 4755 /usr/local/bin/collectd-pm2
   #+end_src
   ...
   #+begin_example
     ...
     <Plugin exec>
         Exec "nobody:nobody" "/usr/local/bin/collectd-pm2"
     </Plugin>
   #+end_example

   测试运行统计收集脚本
   #+begin_src sh
     sudo -u nobody -g nobody /usr/local/bin/collectd-pm2
   #+end_src
