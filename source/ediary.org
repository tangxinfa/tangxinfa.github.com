      ...
      Database.prototype.redis = function(callback) {
          ...
          if (! self._redis) {
              self._redis = new Redis({
                  sentinels: self._redis_sentinels,
...
** IoT领域的其它竞争对手
  ...
  - [[https://tools.ietf.org/html/draft-ietf-httpbis-http2][HTTP/2]]
...
** 个人总结
   ...
   或许将来有一天会出现构建于 [[https://tools.ietf.org/html/draft-ietf-httpbis-http2][HTTP/2]] 上的 =Pub/Sub= 流行解决方案，它能够运行在绝大多数的资源内限嵌入式设备上，在这之前 =mqtt= 之类的成熟的应用层协议会有一席之地，特别是在它完美契合你的应用的情况下。

   做为一个开发者，与其坐等完美的解决方案出现，还不如立即开始学习 =mqtt= ，应用 =mqtt= ，从 =mqtt= 中学到协议设计的经验与技术，以至成为实现构建于 [[https://tools.ietf.org/html/draft-ietf-httpbis-http2][HTTP/2]] 上的 =Pub/Sub= 流行解决方案的那个人。

* TODO node.js下进行mqtt实践                                    :network:IoT:node:
