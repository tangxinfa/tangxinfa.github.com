...
* DONE 使用 NDK 移植 Linux C/C++ 程序到 Android 系统                :android:
...
移 [[https://developer.android.com/ndk/guides/stable_apis][NDK 支持的 API]] 也在增加。
...
~crypt.h~ 头文件而编译不过。如果强行将 ~glibc~ 之类的东西也移植过来我估计后面也
...
** 结论
   ...
   使用了 NDK 不支持的特性（如 ~glibc~ ），应该采用和 Android 源代码一起编译的方
   式，这样可以使用 NDK 不支持的特性。
