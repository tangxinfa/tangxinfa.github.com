...
** Envoy 的 Resiliency Features
   ...
   - [[https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/health_checking#arch-overview-health-checking][Health Checking]] :: 主动检查上游集群（Upstream Cluster）的康健度

   - [[https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/outlier][Outlier Detection]] :: 被动检查上游集群（Upstream Cluster）的康健度
...
*** [[https://www.envoyproxy.io/docs/envoy/v1.7.0/api-v1/route_config/route#config-http-conn-man-route-table-route-timeout][Timeouts]] 、[[https://www.envoyproxy.io/docs/envoy/v1.7.0/intro/arch_overview/http_routing#retry-semantics][Retries]]
    ...
    最基本的保障服务稳定性的特性，主要是为了保护 Envoy 自身，避免跟着被拖垮。
...
*** [[https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/circuit_breaking#arch-overview-circuit-break][Circuit Breaking]]
    ...
    静态地为上游服务的处理能力设定上限，丢弃超过上游服务的处理能力的请求。
...
** Envoy 怎样实现熔断
   ...
   当调用的上游（Upstream）服务运行状态良好时，请求失败是非常偶然的，通过运用
   ~Retries pattern~ 来重试失败的请求，可以提高服务的容错性、健壮性。

   当调用的上游（Upstream）服务运行状态异常时，请求失败会大量出现，通过运用
   ~Circuit Breaker pattern~ 来避免发起有很大机率失败的请求，可以释放系统压力、
   避免“雪崩效应”。
   ...
   - Open :: 熔断器打开状态
             ...
             此时对上游（Upstream）的调用都内部直接返回错误，不会真的发起调用，
             同时开启一个定时器，时长通常为平均故障处理时间（ ~MTTR~ ），定时器
             触发后进入半熔断状态
