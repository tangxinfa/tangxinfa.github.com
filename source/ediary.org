...
* DONE 修复 offlineimap 无法收邮件的问题               :linux:network:python:
  ...
  offlineimap 配置配置的是普通端口（143），但是使用 tls 进行了连接。


  telnet 邮件服务器输出的 CAPABILITY 有 STARTTLS，应该是邮件服务器配置有误
  #+begin_example
  ...
  修改 offlineimap 的源代码文件 [[file:///usr/lib/python2.7/site-packages/offlineimap/imapserver.py][imapserver.py]] ，不使用 tls
  #+begin_src python
