...
* TODO 诊断 node.js 应用 cpu 占用过高的问题                      :node:linux:
...
*** 系统负荷正常
    ...
    =top= 的 =load average= 值远小于 CPU 核数
...
*** CPU 资源占用较高
    ...
    - 中断数偏多
      ...
      应该是网络 I/O 数过多引起，参见 =sar= 的 =iseg/s= =oseg/s= 列。

      中断数多，但并非瓶颈，参见 =top= 的 =%hi= 和 =%si= 值。
...
* DONE collectd 数据类型详解                                       :collectd:
  ...
  #+end_quote
  #+begin_quote
  type 标识传给 collectd 的值（如：数据集）的类型和数量。types.db 中预定义了大量数据集（data-sets）。types.db 的格式描述可以查看 types.db(5) 手册。
  #+end_quote
  ...
  DERIVE
      ...
      对于很少溢出（达到最大值后回绕）的计数器推荐使用 DERIVE 数据源并设置最小值（min）为 0。
...
** 示例
   ...
   插件名称 =test_load= ，数据类型（数据集）为 =load= ，shortterm、midterm、longterm的值分别为 1、2、3。
