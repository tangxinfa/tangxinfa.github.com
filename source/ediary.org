...
* DONE 使用node.js的对象模式验证模块joi引入强类型                      :node:
  ...
  - 弱类型的Javascript
    ...
    Javascript是一门弱类型的语言，定义变量不需要指定类型，可以为同一个变量赋任意类型类型的值。程序中误用类型程序不报错而是意外的结果：
    ...
    redis中很多数据结构取出时都是字符串值（如：set、hash），调用方需要自行将它转换成正确的类型（如：Boolean、Date、Number），如果仍然使用字符串类型会导致很多的冗长的代码，如Boolean类型：
  ...
  - 对象模式验证模块joi
    ...
    有很多的ORM（ Object Relational Mapping 对象关系映射）库都能够实现强类型的数据模型，但是它们都相当的复杂，支持各种各样的数据库后端，支持一对一、一对多、多对多等数据关系，但是很少支持分表分库，我们的系统一般都要数据模型简单但要考虑用户量大了后如何横向扩展，所以从一开始就是分表分库的，不会使用重型的ORM。

    一个单纯的对象模式验证模块能够满足需要并且适用范围更广。而今天要提的就是 [[https://github.com/hapijs/joi#anydefaultvalue-description][joi]] ： Object schema description language and validator for JavaScript objects。
