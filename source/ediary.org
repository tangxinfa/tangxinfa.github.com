...
*** 创建Master结点
    ...
    - redis配置文件 :: [[file:../static/redis-master.conf][redis-master.conf]]
      
                       从 =/etc/redis.conf= 拷贝一份进行修改，无关的配置项已去掉，使用默认值即可。

    - 在终端 =1= 启动 =Master= 结点 :: =redis-server ./redis-master.conf=
...
* TODO redis高可用性方案：Sentinel                                    :redis:
* TODO node.js连接redis高可用性方案：Sentinel                    :redis:node:
