...
* DONE linux下翻墙访问bitbucket.org仓库                  :linux:hg:bitbucket:
  ...
  今天往bitbucket.org push时才发现bitbucket被GFW了。我的仓库为Mercurial hg，hg项目根目录下的 =.hg/hgrc= 配置文件中可指定http_proxy，试了一下不支持socks代理（我的浏览器用它来翻墙），最终使用tsocks软件实现翻墙访问bitbucket.org仓库。
...
* DONE 使用Pdnsd缓存域名解析结果加快上网速度                        :web:dns:
...
* DONE 后台服务监护工具：forever与pm2                                  :node:
...
* DONE process.exit导致的控制台输出不全                                :node:
  ...
  - =Emacs Shell= 以及 =GNOME Terminal= 下运行结果（省略中间部分输出）：
  ...
  - 按 =Ctrl+Alt+F2= 进入2号系统终端下运行结果（省略中间部分输出）：
  ...
  - 日志重定向到文件时 =Emacs Shell= 以及 =GNOME Terminal= 下运行结果（省略中间部分输出）：
  ...
  - 原因
    ...
    如果输出目标为终端（ =Emacs Shell= 以及 =GNOME Terminal= 并非真正的终端）或文件时，console函数是同步的（避免程序过早退出导致丢消息），目标为管道时则为异步（避免长时间堵塞）。

    #+begin_quote
    The console functions are synchronous when the destination is a terminal or a file (to avoid lost messages in case of premature exit) and asynchronous when it's a pipe (to avoid blocking for long periods of time).
    #+end_quote

    参考：《[[http://stackoverflow.com/questions/18748164/process-exit0-output-disappears][process.exit(0): output disappears?]]》

  - 解决方案

    调用 =process.exit= 时确保输出完毕，封装如下：

    #+begin_src js -n -r
      /// Safe exit. Makesure log output flushed to console.
      function exit(code) {
    ...
    注意，上面的 =exit= 函数不会像 =process.exit= 一样立即终止程序，请将 =process.exit= 替换为  =return exit= 。

    通过 [[https://github.com/Unitech/pm2][pm2]] 运行 node.js 应用时会接管标准输出和错误输出到日志文件，不存在进程退出日志输出丢失的问题，
    由于 [[https://github.com/Unitech/pm2][pm2]] 会改写 process.stdout.write 和 process.stderr.write ，改写后的函数无返回值并忽略传入的回调函数，
    导致前面封装的 exit 函数无效，需要针对 [[https://github.com/Unitech/pm2][pm2]] 做处理：

    #+begin_src js
      /// Safe exit, Makesure log output flushed to console.
      /// PM2 changed process.stderr.write and process.stdout.write's behavior,
      /// log output write to file will flushed automatically,
      function exit(code) {
...
* DONE 修复文件目录权限引起的错误：Cannot find module                  :node:
  ...
  这是延迟加载模块，代码本身没有错， =../encodings= 模块是存在的，以前也有遇到类似问题，不过是在 =node.js= 一启动就报错，因为系统默认的权限太严格了（如： =umask= 为 =0022= ），
  =npm install= 安装的 =node_modules= 其它用户（如： =nobody= ）没有访问权限。这次是在处理用户请求过程中报错，考虑到上面的模块是延迟加载，
  在初始化完成与监听端口处理用户请求的代码之间，我的代码有 =setuid= 到 =nobody= 的逻辑，应该还是文件目录权限引起。

  最终发现是 =/usr/local/www.xxxxxxxx.com= 在创建时因为系统默认的权限设置太严格（ =umask= 为 =0022= ），导致 =nobody= 用户无法访问，改一下目录权限后问题修复。
...
* DONE 我应该是中度抑郁症                                                :心理:
  CLOSED: [2016-01-07 Thu 16:44]

  一直怀疑自已有抑郁症，今天看了一下《走出抑郁的阴影：伯恩斯新情绪疗法》，根据里面提及的“贝克抑郁清单”进行了自测，结果为 =24分 中度抑郁= 。

  以前听到电视里提到绝大部分人都有心理疾病，我总是做心里点头默默飘过状，想不到自已也会中招。

  人或许就像是程序，上帝写出来的时候是完美的，只是成长过程中掺入了杂质，出现了 =BUG= ，我要开始 =DEBUG= 了。
* DONE 我的抑郁症的运行过程                                              :心理:
  CLOSED: [2016-01-09 Sat 18:11]
...
** 习惯的力量
   ...
   之前有看过《习惯的力量》，我觉得抑郁症就是一种心理的坏习惯，就算你发现了它，了解了它，克服它绝非一朝一夕，因为它是自发运行的，在你意识到它之前它可能已经运行了很久，对你的身心造成了很大的破坏。它就像是蚂蟥，它刚开始叮上你的时候，会稍微痛一下，等到它释放出麻醉效果，你就再也感觉不到它在吸你的血，甚至在往你身体里钻。抑郁症也会麻醉你的感知，让你默默长久承受心灵的痛苦，你习惯了这种痛苦，你又有抗拒改变的本能。就像是这个世界上难以戒除的各种瘾一样，抑郁症也是人生中要迈过的一道坎。
...
* DONE git多人协作：维护干净的提交树                                    :git:
  ...
  接收方随时可以将补丁合并进来，完全没有 merge 模式合并补丁那么麻烦。强烈建议默认情况下使用这种模式。
...
* DONE CentOS 6.4 生产环境上安装 pm2                             :node:linux:
...
* DONE Module version mismatch 错误排查                                :node:
...
** 获取 CA 证书
   ...
   linux 桌面的 CA 证书由操作系统本身负责维护，没有统一的标准规定如何维护和管理 CA 证书。

   比较有名的 CA 证书列表由 mozilla 维护，[[https://curl.haxx.se/][curl]] 提供了命令行工具 [[https://github.com/curl/curl/raw/master/lib/mk-ca-bundle.pl][mk-ca-bundle.pl]] 用于下载 mozilla 维护的 CA 证书列表并转换成 SSL 的应用程序直接使用的格式。
   ...
   拷贝 Archlinux 的 CA 证书文件 /etc/ssl/certs/ca-certificates.crt 到嵌入式 linux 设备，就可以通过 curl 访问 https 站点。
   由于嵌入式 linux 设备上 curl 使用的默认 CA 证书文件路径不同，需要通过 --cacert 选项明确指定
   #+begin_example
...
** 更新 CA 证书
   ...
   使用 openwrt 的嵌入式设备，可以通过 opkg 进行安装和更新 CA 证书文件
   #+begin_src sh
   ...
   使用 linux 的嵌入式设备，如果没有软件包管理器，可以从 linux 服务器上定时下载最新的 CA 证书文件。

   linux 服务器上的 CA 证书文件可以通过两种方式生成：

   - linux 服务器自动更新 ca-certificates 软件包，确保 CA 证书文件保持最新。
   
   
   - [[https://github.com/curl/curl/raw/master/lib/mk-ca-bundle.pl][mk-ca-bundle.pl]] 脚本生成 CA 证书文件 
