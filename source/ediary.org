...
[[https://developer.android.com/ndk/guides/stable_apis][NDK 支持的 API]] 也会随着需求的增加而日趋完善，比如有人提 ISSUE（[[https://github.com/android-ndk/ndk/issues/718][Undefined
reference to glob and globfree in libc.so · Issue #718 · android-ndk/ndk]] ），然
后在接下来的版本 ~r17b~ 中就添加了 ~glob~ 的支持，可以期待以后使用 NDK 移植程序
到 Android 平台会越来越方便。
...
- 创建独立工具链
  ...
  #+begin_src sh
    /opt/android-ndk-r16/build/tools/make_standalone_toolchain.py \
...
- 环境变量配置
  ...
  #+begin_src sh
    ...
    export CROSS_COMPILE_SYS_ROOT=/opt/android-toolchain/sysroot
  #+end_src
...
- 交叉编译 OpenSSL
  ...
  [[https://github.com/couchbaselabs/couchbase-lite-libcrypto][couchbaselabs/couchbase-lite-libcrypto: Pre-built OpenSSL libcrypto static
  libraries]] 移植成功。OpenSSL 的交叉编译需要完整的 NDK 包，最好新开一个 Shell 来
  编译 OpenSSL，避免为独立工具链设置的环境变量影响到 OpenSSL 的编译。
  ...
  #+begin_src sh
    ...
    ./Configure no-ssl2 no-ssl3 no-comp no-hw no-engine --openssldir=/opt/local/ --prefix=/opt/local/ linux-generic64 -DB_ENDIAN -B$ANDROID_DEV \
                -fPIE -fPIC -I${ANDROID_NDK_ROOT}/sysroot/usr/include -I${ANDROID_NDK_ROOT}/sysroot/usr/include/${ANDROID_EABI_PREFIX} \
                -pie -L${ANDROID_NDK_ROOT}/platforms/${_ANDROID_API}/${_ANDROID_ARCH}/usr/lib
...
Nginx 没有使用 GNU Autotools，而是有自已的 Configure 脚本，遇到的第一个问题就是
Nginx 是通过使用编译器编译一个测试程序来获取配置信息，交叉编译出来的程序肯定是无
法在编译机上运行的，一个可靠的办法就是修改 Configure 脚本，改为上传到目标设备上
执行。
...
#+begin_src sh
  ...
  if [[ "$CROSS_COMPILE_HOST" = *"android"* ]]; then
      ...
      if [ $1 == "-f" ]; then
          ...
          adb shell "find ${tempdir} -mmin +1 -type f -exec rm {} \; >/dev/null 2>&1; cd ${tempdir} && chmod a+x $tempfile && ./$tempfile"
      else
          adb shell $2
      fi
  else
...
~crypt.h~ 属于 ~glibc~ ，NDK 没有包括进来，可以将 ~crypt~ 调用替换为 ~DES_crypt~
#+begin_src sh
...
NDK 工具链是有提供 ~glob.h~ 头文件的，但是链接时却找不到 ~glob~ 函数。网络上有很
多单独提供 [[https://github.com/tatowilson/Cross-Compile-Nginx-with-RTMP-Module-for-Android/tree/master/glob][glob]] 下载的，但是都无法直接通过编译，因为里面有很多平台相关的特性。
NDK 有人提了一个 ISSUE （[[https://github.com/android-ndk/ndk/issues/718][Undefined
reference to glob and globfree in libc.so · Issue #718 · android-ndk/ndk]] ），说
是已经在 ~r17b~ 版本解决，升级 NDK 到最新的 ~r17-beta2~ 还必须将 ~API Level~ 设
置为 ~28~ 才能解决这个问题，这就意味着编译出来的程序无法在旧版本的
Android（<=8.1） 上运行。
...
能是因为 Android 安全沙箱需要或者版权限制。

- 没有使用 ~glibc~

~glibc~ 是 Linux 下的 C 标准库实现，除了实现 ~ANSI C~ 标准库还有大量方便 Linux
开发的扩展功能。而 NDK 提供的 C 标准库并非 ~glibc~ 。这导致移植 Nginx 时由于缺少
~crypt.h~ 头文件而编译不过。

NDK 将在未来的版本中移除 ~gcc~ 而只支持 ~clang~ ，已知 Standalone 工具链中使用
~gcc~ 会 [[https://github.com/android-ndk/ndk/issues/215][出现编译错误]]：
#+begin_quote
cstddef:43:25: fatal error: stddef.h: No such file or directory
#+end_quote
初步使用 ~clang~ 移植了一些库（如 [[https://sourceforge.net/projects/pcre/][pcre]] ）是没有问题的。
...
** 结论
   ...
   NDK 一直在改进，以前阻碍我们移植到 Android 的问题很可能会在新版本中解决。
