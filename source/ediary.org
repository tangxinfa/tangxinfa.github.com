...
- 交叉编译出的配置检测程序无法直接运行
  ...
  #+begin_src sh
    sed -i -e 's/\/bin\/sh -c $NGX_AUTOTEST/\/build\/execute -f $NGX_AUTOTEST/g' `find auto -type f`
    sed -i -e 's/$NGX_AUTOTEST >\/dev\/null/\/build\/execute -f $NGX_AUTOTEST >\/dev\/null/g' `find auto -type f`
    sed -i -e 's/`$NGX_AUTOTEST`/`\/build\/execute -f $NGX_AUTOTEST`/g' `find auto -type f`
  #+end_src
