...
** collectd 的四种数据类型
   ...
   假设要上报一个 100/秒 的统计值，在一个时间间隔（10 秒）内分成两部分上报，分别使用四种数据类型进行上报。
   ...
   =ABSOLUTE= 类型值的处理
   #+begin_quote
   将时间间隔（10 秒）内的值进行累加，然后与时间间隔（10 秒）相除
   ...
   =COUNTER= 类型值的处理
   #+begin_quote
   将时间间隔（10 秒）内的值进行累加，减去上一时间隔的累加值，然后与时间间隔（10 秒）相除

   ((1000 + 900 + 100) - 1000) / 10 == 100
   #+end_quote
   ...
   #+begin_src sh
    ...
    while [ 1 ]; do
        sleep 9
        VALUE=$[VALUE+900]
        echo "PUTVAL \"$HOSTNAME/test/derive\" N:$VALUE"
        sleep 1
        VALUE=$[VALUE+100]
        echo "PUTVAL \"$HOSTNAME/test/derive\" N:$VALUE"
    done
...
** =COUNTER= VS =DERIVE=
   ...
   这两者非常相似，不同点表现在当前时间间隔（10 秒）的值小于上一时间间隔（10 秒）的值时：
   ...
   - =DERIVE= 认为这是异常值，丢弃该值，当前时间间隔（10 秒）的值为 UNKNOWN 。


** =GAUGE= 与 =ABSOLUTE= 的特殊之处

   =GAUGE= 和 =ABSOLUTE= 都不依赖上一时间间隔的值。

   =GAUGE= 已经是变化率值（数量/秒），其它类型需要 RRDtool 进行累加再除以时间间隔算变化率。
