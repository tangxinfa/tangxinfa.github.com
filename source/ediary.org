...
** 解决方案
   ...
   - 使用 fs.writeSync
     ...
     process.stdout 和 process.stderr 流中滞留的数据没有机会输出，进程就退出了，只能保证使用 fs.writeSync 进行输出的日志不丢失，用来日志记录 uncaughtException 还是很合适的。

     使用 pm2 时，fs.writeSync 写的异常日志会出现在 /root/.pm2/pm2.log，可以考虑改用 fs.appendFileSync 将异常日志写到应用的日志文件中。
     ...
     服务往往为了安全起见切换（setuid、setgid）到非特权帐号，可能没有权限写 pm2 为应用创建的日志文件。
...
* TODO 高可用后台服务的系统思维                                :architecture:
...
** 授权（Authorization）接口原型
   ...
   #+begin_src C
     /**
      ,* Authorize user to access resource.
      ,*
     ...
     /**
      ,* Deauthorize user from access resource.
      ,*
      ,* @param user - User to deauthorize.
      ,* @param resource - Resource to deauthorize.
      ,*
      ,*/
     void deauthorize(User user, Resource resource);

     /**
      ,* Is user authorized on resource.
      ,*
      ...
      ,*/
     bool authorized(User user, Resource resource);
   #+end_src

   - =authorize= :: 允许用户访问资源
     
   
   - =deauthorize= :: 禁止用户访问资源


   - =authorized= :: 检查用户是否允许访问资源
...
* DONE 使用 npm scripts 统一操作接口                                   :node:
  ...
  - .bashrc
    ...
    设置环境变量，由于一台机器上可能跑多个服务，而每个服务的环境设置会不一样，所以我们不好直接放置在 =~/.bashrc= 中。
  ...
  - stop.sh
    ...
    会先载入 .bashrc 中定义的环境变量，再使用 =pm2 stop <app>= 启动服务
  ...
  还有一堆 js 任务脚本，你得决定要不要提供配套的 bash 脚本（确保载入 .bashrc），或者祈祷运行前操作人员会记得载入 =.bashrc= 环境变量。
