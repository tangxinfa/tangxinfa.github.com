...
** [[https://github.com/Unitech/pm2][pm2]]
   ...
   - 缺点
     ...
     + 程序退出过程中的日志无法捕获？
       ...
       使用 =pm2 stop= 会有同样的问题，但是pm2支持优雅退出（ =pm2 gracefulReload= ），此时不但退出过程中的日志能够正常捕获，而且可以实现服务0停机时间。
       
     + 重启可能失败？

       =pm2 restart= 并没有采用激进的措施（kill -9）确保旧进程结束。重现步骤：用gdb调试运行中的node进程（gdb node <PID>后不执行任何gdb命令），然后用pm2 restart重启服务，此时旧的进程杀不死，新的进程被创建。
