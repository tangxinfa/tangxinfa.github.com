...
** redis 至少要求 2.8.12
   ...
   redis 2.8.12 实现了一个特性：当 failover （redis 角色变化）后，断开所有 client 的连接。缺少这个特性会导致 failover 发生时到 master 的没有断开而是返回返回 READONLY 错误，可以通过 reconnectOnError 选项判断为 READONLY 错误后触发重连，但是将 redis 用来做 PUB/SUB 时不会出现 READONLY 错误，所以一定要升级到至少 2.8.12 以上。
...
** 在 2.4.x 的 ioredis 上实现优先选择本地结点（使用 preferredSlaves）
   ...
   - 使用 preferredSlaves 连接距离最近的 slave


   - 如果 slave 是本地结点，则使用该 slave 连接；否则尝试连接 master

     
   - 如果 master 是本地结点，则使用该 master 连接，否则使用前面的 slave 连接。


   这导致当本地无 slave 而连上本地 master 后，下次重连时即使本地有 slave 了，也还是会继续重连 master。
...
** 在 2.4.x 的 ioredis 上实现优先选择本地结点（不使用 preferredSlaves）
   ...
   - 如果 slave 已经是本地结点，则使用该 slave 连接；否则尝试连接 master
