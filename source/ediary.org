...
** Envoy 怎样实现熔断
   ...
   一方面，通过对 Envoy 的 [[https://www.envoyproxy.io/docs/envoy/v1.7.0/api-v1/route_config/route#config-http-conn-man-route-table-route-timeout][Timeouts]] 、[[https://www.envoyproxy.io/docs/envoy/v1.7.0/intro/arch_overview/http_routing#retry-semantics][Retries]] 、[[https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/circuit_breaking#arch-overview-circuit-break][Circuit Breaking]]、[[https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/other_features/global_rate_limiting#arch-overview-rate-limit][Rate
   Limiting]] 合理配置来尽量保障上游（Upstream）服务的稳定运行；另一方面，一旦通
   过 [[https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/health_checking#arch-overview-health-checking][Health Checking]]、[[https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/outlier][Outlier Detection]] 检测到上游（Upstream）服务主机（Host）
   异常，就会进入熔断状态。
...
** 从熔断到降级
   ...
   熔断和降级是非常容易混淆的概念，它们往往同时出现，在一定程序度可以认为熔断是
   一种自适应式的降级。
