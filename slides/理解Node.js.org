...
* Node.js的Web框架：Connect

  #+ATTR_HTML: style="float:left; margin-left:1em; width:10em;"
  [[../static/dry.jpeg]]
  
  #+begin_quote
  [[http://www.senchalabs.org/connect/][=Connect=]] 是 [[http://nodejs.org/api/http.html#http_http][=http=]] 模块之上的一层封装\\
  是一个插件式HTTP服务器框架\\
  大量内置中间件及 [[https://github.com/senchalabs/connect/wiki][第三方中间件]]\\
  目标是成为高级Web框架的基石
  #+end_quote
...
* Connect框架工作原理
  ...
  =use= 方法的原型“ =function= ( =route= , =fn= )”，用于在匹配的请求路径上挂载中间件。
  #+ATTR_HTML: style="border-style:hidden;"
  | =route= | 为请求路径，可选，默认为 =/= ，匹配所有请求路径。 |
  | =fn=    | 为中间件                                          |
  

  使用 =route= 参数限定中间件处理范围：
  #+begin_src js
...
* Node.js使用Connect库构建Http服务
* Node.js使用Express框架构建Http服务
* Node.js超越Http服务
* Javascript工具：Jslint
* 浏览器中的Javascript
* C10K问题
