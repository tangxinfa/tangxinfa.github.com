...
*** 发现问题
    ...
    [[~/Projects/tangxinfa.github.com/static/site_attacked.png]]
...
*** 解决问题
    ...
    - 打开 =firebug= 的 =Network= 标签，在页面加载记录中发现一个异常的js： =http://api.discuz.com.de/Seo.js= 
    ...
    - 从 =google= 跳转到论坛后，查看页面源代码在最开始部分发现了引入这个js的语句。
...
*** 分析恶意代码
    ...
    - 参考 [[http://blog.i1728.com/post/99.html][网上的方法]] 解码的结果
...
*** 继续排查
    ...
    将通过比较svn中的代码与现网的代码还发现了一个新的后门程序
...
*** 总结
    ...
    这次发现的恶意代码注入应该是之前的一次 =nginx= 和 =php= [[http://www.oschina.net/translate/10-tips-for-securing-nginx#translate_28443][安全漏洞]] 引起，当时由 =WooYun.org= 汇报，也不知道已经被恶意用户种下了这些后门。
