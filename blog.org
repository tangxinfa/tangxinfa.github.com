...
** DONE 为什么不能在构造函数中调用shared_from_this                      :cpp:
   ...
   _weak_ptr为父类（enable_shared_from_this<Chicken>）的成员变量，需要一个shared\_ptr<Chicken>对象来初始化它，而shared\_ptr<Chicken>需要一个Chicken对象来创建，但此时Chicken对象正在构造中，这是个鸡与蛋的无解问题。
