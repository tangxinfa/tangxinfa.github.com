...
** DONE SSL_read及SSL_write支持超时                                   :openssl:c:
   ...
   - 启用自动重新握手
     ...
     SSL\_read或SSL\_write可能因为SSL需要重新握手而提前返回（此时SSL错误码为SSL\_ERROR\_WANT\_READ或SSL\_ERROR\_WANT\_WRITE），也可能因上面的socket超时设置而提前返回，通过启用自动重新握手，消除第一种情况。

     #+begin_src c
       SSL_CTX_set_mode(ctx, SSL_MODE_AUTO_RETRY);
     #+end_src

   - SSL\_read和SSL\_write判断是否超时出错
