...
*** 问题
    在web开发中，以典型的php-fpm为例，对于到外部系统的连接（如：mysql、redis）等都提供了持久连接接口（pconnect），但是受限于多进程模型，事实上是每个php-fpm进程都有单独的一个连接池的（参见：《[[file:php_meet_redis.org][当php遇上redis]]》），大量空闲连接的存在不仅对系统资源造成了浪费（不单指fd空间，像mysql的每连接一线程会附带大量内存空间：sort_buffer、read_buffer等），而且整个系统将无法横向扩展（如：mysql连接数限制）。如果可以在进程间共享文件描述符，将可以大大提升系统性能，促进多进程模型的应用。
...
** DONE SSL_read及SSL_write支持超时                                   :openssl:c:
   ...
   - 启用自动重新握手
     ...
     =SSL_read= 或 =SSL_write= 可能因为SSL需要重新握手而提前返回（此时SSL错误码为 =SSL_ERROR_WANT_READ= 或 =SSL_ERROR_WANT_WRITE= ），也可能因上面的socket超时设置而提前返回，通过启用自动重新握手，消除第一种情况。
...
** DONE Node.js服务器TCP死连接问题诊断                     :node:network:
   ...
   TCP死连接一般在一方（或中间线程上的设备）断电、死机后出现，此时由于另一方收不到断开连接的IP报文，会认为连接仍然存在，日积月累会耗光文件描述符空间从而导致性能下降，最终拒绝服务。
   ...
   除了常见的断电、死机引起TCP长连接外，这里还有一个论坛帖子论坛其它原因：《[[http://serverfault.com/questions/504187/too-many-established-connections-left-open][Too many established connections left open]]》。

   另外还有 linux 内核的 tcp keepalive机制作为辅助解决方案：《[[http://machael.blog.51cto.com/829462/211989/][linux下使用TCP存活(keepalive)定时器]]》。
