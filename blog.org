...
*** 挂载模块
    ...
    - 修复挂载错误
      ...
      #+begin_quote
        insmod: ERROR: could not insert module src/flashcache.ko: Unknown symbol in module
      #+end_quote
      ...
      挂载 =dm-mod= 模块 再挂载 =flashcache= 模块即可：
...
*** 模拟实验
    ...
    - 创建SSD模拟设备
      ...
      #+begin_src sh
        dd if=/dev/zero of=/dev/shm/ssd.img bs=1024k count=200
    ...
    - 创建HDD模拟设备
      ...
      #+begin_src sh
        dd if=/dev/zero of=/tmp/hdd.img bs=1024k count=1024
        sudo losetup /dev/loop2 /tmp/hdd.img
      #+end_src
    ...
    - 挂载Flashcache混合设备
      ...
      #+begin_src sh
        mkdir /tmp/cachedev
        sudo mount /dev/mapper/cachedev /tmp/cachedev
      #+end_src

    现在可以试试在/tmp/cachedev目录下读写数据，感受Flashcache的效果。
