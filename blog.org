...
** TODO CentOS 6.4下安装redmine                               :linux:redmine:
...
*** 启用 =OpenID= 支持第三方帐号登录
    ...
    - 安装插件简化 =OpenID= 登录
      ...
      + [[https://github.com/jorgebg/redmine-openid-selector]] 为原始分枝，在 =redmine-2.5.0= 下不能直接安装会导致站点登录界面出现404错误
        
        解决方案在 [[http://www.redmine.org/boards/3/topics/34327?r=38778#message-38778][这里]] ，简而言之就是把插件目录名中的 =-= 改为 =_= 。
        
      + [[https://github.com/computerminds/redmine_openid_selector]] 这个分枝安装后可用，但界面为英文（其实界面就一句英文）。
      
      + https://github.com/tangxinfa/redmine_openid_selector 上面的分枝为了支持中文我新建的分枝。
